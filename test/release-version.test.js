...
test("current Windows, Android, and Band packages share the formal 0.6.5 version", async () => {
  ...
  assert.match(androidGradle, /versionName\s*=\s*"0\.6\.5"/);
  assert.match(
    androidGradle,
    /versionCode\s*=\s*607/,
    "the 0.6.5 candidate keeps a monotonic Android install sequence",
  );
  assert.equal(bandPackage.version, releaseVersion);
  assert.equal(bandManifest.versionName, releaseVersion);
  assert.equal(bandManifest.versionCode, 607);
});
