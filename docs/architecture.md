...
## 1.1 当前正式版本状态（2026-08-04）

三端当前验收状态、临时版本差异和未决产品冲突以 [current-status.md](current-status.md) 为准。本节只记录
当前代码和协议事实，不能据此覆盖 `CONTEXT.md` 中已经确认的产品交互。

- 当前代码与配置基线：产品版本 `0.6.3`，Windows、Android APK 和手环 RPK 按同一产品版本管理；Android 与手环内部安装序号均为 `605`，用于在不丢失本地数据的情况下覆盖此前版本。
- 自动化验证和产物哈希以 [build-verification.md](build-verification.md) 中的当前 `0.6.3` 发布候选记录为准。
...
- 验收与发布状态：`0.6.3` 已完成自动验证和三端真机验收，正式 GitHub Release 尚未创建。
...
- **同步与状态**：WSS 在线本身不等于额度刚刚确认。新连接完成协议协商后立即请求一次当前额度确认；此后客户端每 45 秒请求一次刷新，并每 5 秒重新评估年龄。新鲜度在 60 秒内显示“已同步”，超过 60 秒转为“缓存”，离线显示“离线”。该刷新依赖进程存活，不承诺被 Android 杀死后仍持续执行。
...
- `runtime/SyncWebSocketClient` / `SyncStreamSession`：固定电脑公钥、连接 `/sync`、重连、序列号和快照去重；
  新连接协商完成后立即发送一次已认证的额度刷新请求，连接存活时每 45 秒继续刷新，并每 5 秒复核上游确认的年龄。
...
- 手环额度摘要使用 Android 侧独立版本 2；重置卡不含 v1 的 `id`、`title` 或 `description`。`0.6.3`
...
`src/`、`astrobox-plugin/` 和部分 ADR/验收记录包含 0.4.0 以前的 Electron/AstroBox 架构。它们用于历史回溯，不是当前 0.6.3 主构建、日常链路或发布产物。除非用户明确要求维护旧版，不要把旧端口、旧 Bearer 协议或 AstroBox 日常桥接重新接入新实现。
