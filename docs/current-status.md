...
当前正式发布版本为 `0.6.2`；当前已验收发布候选为 `0.6.3`，Android 与手环内部安装序号均为 `605`，用于在不丢失本地数据的情况下覆盖此前版本。`0.6.3` 的正式 GitHub Release 尚未创建。
...
- 安卓“需要授权”和“等待查看”已改用请求默认振动、无声音的新通知通道；手机 App 自身位于前台不再吞掉提醒。提醒设置已精简为通知时机、手机通知、手环通知和系统通知设置，三档副标题随选择动态变化，分类级调整交给 Android 系统。自动测试、正式构建、覆盖安装和真机 UI 检查均已完成。
- 三端版本声明已统一为 `0.6.3`，Android 与手环内部安装序号为 `605`。三端正式产物已通过自动验证并完成覆盖安装，最终真机联动验收通过；正式 GitHub Release 尚未创建。
...
`0.6.2` 已于 2026-08-04 正式发布。`0.6.3` 当前已通过三端真机验收，正式 GitHub Release 尚未创建；Release 操作仍需用户另行明确授权。
