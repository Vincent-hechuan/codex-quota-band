...
这是 Codex额度 0.6.5 的原生 Windows 端实现。托盘、额度采集、加密局域网服务、配对持久化和只读任务 Hook 已接入。
...
- 主程序、原生窗口、安装/卸载界面和开始菜单快捷方式统一使用内嵌的多尺寸产品图标；构建时会检查 PE 图标资源，避免只在开发机托盘中可见。
...
- 配对窗口同时展示二维码、6 位一次性配对码和 `XXXX-XXXX` 安全校验码。Android 可在 App 内扫码；无法扫码时自动发现同一局域网中的候选电脑，用户核对安全校验码后输入 6 位数字，不需要填写局域网地址。窗口保持单实例；重复点击托盘入口只置前，只有“刷新配对码”会更新当前凭据。
...
- 输出与 Android 严格解析器一致的额度 Snapshot v1/v2/v3、任务 Task Sync v1 和加密同步流
  v1 白名单消息；额度版本由连接协商，已接入 TLS 1.3 WebSocket `/pair` 与 `/sync` 服务，支持一次性短码、
...
- 从持久身份构造仅启用 TLS 1.3 的 Rustls 服务配置，并声明 WebSocket 所需的
  ...
  Windows 托盘启动、真实额度采集和独立运行安装包均已接入。0.6.5 当前为本地待验收候选。

## 本地测试

本项目使用 Rust GNU-LLVM Windows 工具链，以避免依赖 Visual Studio 或浏览器运行时。
该工具链还需提供 `windres`，用于把应用图标编译进主程序资源。
